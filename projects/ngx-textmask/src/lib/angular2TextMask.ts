...
export class MaskedInputDirective implements ControlValueAccessor, OnChanges {
  ...
  _handleInput(target: EventTarget | null) {
    ...
    if (!this._compositionMode || (this._compositionMode && !this._composing)) {
      ...
      if (this.textMaskInputElement !== undefined) {
        this.textMaskInputElement.update(input.value);
  ...
  _compositionEnd(target: EventTarget | null): void {
    this._composing = false;
    const input = target as HTMLInputElement | null;
    if (this._compositionMode && input) {
