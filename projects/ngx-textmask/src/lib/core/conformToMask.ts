...
export default function conformToMask(rawValue = emptyString, mask: any, config: Config = {}) {
  if (config.currentCaretPosition === null) {
    config.currentCaretPosition = 0;
  }
  if (!isArray(mask)) {
      ...
      if (rawValueArr.length > 0) {
        ...
        while (rawValueArr.length > 0) {
          // Let's retrieve the first user character in the queue of characters we have left
                    const rawValueCharResult = rawValueArr.shift();
          if (!rawValueCharResult) {
            continue;
          }
          const {char: rawValueChar, isNew} = rawValueCharResult;
