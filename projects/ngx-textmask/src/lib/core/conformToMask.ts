...
export default function conformToMask(rawValue = emptyString, mask, config = {}) {
  ...
  const guide = config['guide'] ? config['guide'] : true;
  const previousConformedValue = config['previousConformedValue'] ? config['previousConformedValue'] : emptyString;
  const placeholderChar = config['placeholderChar'] ? config['placeholderChar'] : defaultPlaceholderChar;
  const placeholder = config['placeholder'] ? config['placeholder'] : convertMaskToPlaceholder(mask, placeholderChar);
  const currentCaretPosition = config['currentCaretPosition'] ? config['currentCaretPosition'] : 0;
  const keepCharPositions = config['keepCharPositions'] ? config['keepCharPositions'] :false;

  // The configs below indicate that the user wants the algorithm to work in *no guide* mode
  const suppressGuide = !guide && previousConformedValue !== undefined
