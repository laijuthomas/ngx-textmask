...
interface CreateTextMaskInputElementArgs {
...
export default function createTextMaskInputElement(config: CreateTextMaskInputElementArgs) {
  // Anything that we will need to keep between `update` calls, we will store in this `state` object.
  const state: { previousConformedValue?: string; previousPlaceholder?: string } = {previousConformedValue: undefined, previousPlaceholder: undefined};
  ...
  return {
    ...
    update(rawValue: any, {
      ...
      const {conformedValue} = conformToMask(safeRawValue ? safeRawValue.toString() : '', mask, conformToMaskConfig);
      ...
      // the caret position. `adjustCaretPosition` will tell us.
            const adjustedCaretPosition = placeholder ? adjustCaretPosition({
        previousConformedValue,
        previousPlaceholder,
